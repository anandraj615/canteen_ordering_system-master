...
def index(request):
    food_name_query = request.GET.get('food_name', '')

    # Filter FoodItem objects based on the search query
    food = FoodItem.objects.filter(Q(name__icontains=food_name_query))
    ...
    return render(request, 'order/index.html', {'food': food, 'cartitems': name_quantity_of_all_food, 'food_name_query': food_name_query})
...
def user_logout(request):
    if request.method == 'POST':
        ...
        return redirect('index')  # Redirect to the home page or any other desired URL
    else:
